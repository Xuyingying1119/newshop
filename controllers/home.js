...
exports.index = (req, res, next) => {
    ...
    // 渲染轮播图     获取数据
    // 渲染猜你喜欢   获取数据
    // 两个 Promise 对象同事执行且 监听到最慢的那个执行完成 获取所有的结果
    // Promise.all([Promise对象,Promise对象]) 并且执行多个Promise最慢的结束 执行then()
    // Promise.race() 并且执行多个Promise最快的结束执行 TouchEvent()
